...
function renderCakeQuote() {
  ...
  renderBreakdown($("#cakeBreakdown"), [
    { label: `Tamaño: ${data.serving.people}`, value: formatMoney(data.serving.price) },
...
function renderDesserts() {
  ...
  if (state.dessertsCart.length === 0) {
    ...
    list.textContent = "Aún no agregaste postres.";
  } else {
...
function renderOrderSummary() {
  ...
  sections.push(`
    <section>
      ...
      <div>Teléfono: ${escapeHTML(cake.clientPhone || "No indicado")}</div>
  ...
  if (cake.includeCake) {
    sections.push(`
      <section>
        ...
        <div>Tamaño: ${escapeHTML(cake.serving.people)}</div>
...
function buildWhatsappMessage(cake, buffet, dessertsTotal, finalTotal) {
  const blocks = [
    ...
    `Teléfono: ${cake.clientPhone || "No indicado"}`,
...
function validateOrder() {
  ...
  if (phoneDigits.length < 9) {
    ...
    $("#phoneError").textContent = "Teléfono obligatorio.";
